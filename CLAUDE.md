...
- **✅ RESOLVED — the homepage "Schedule a Demo" contact form and the free Brand Kit lead form now POST to `/api/contact` (Vercel function → Resend), not FormSubmit.** FormSubmit.co was removed entirely (it had a global outage and is an unreliable free third party). Resend is configured (`sicusmedia.com` domain verified + `RESEND_API_KEY` set in Vercel), and `/api/contact` was verified live returning `200 {"success":true}`. All email (lead magnets + contact + brand kit) now goes through Resend. `/api/contact` delivers to `marketingsicusmedia@gmail.com` (override with the `CONTACT_TO` env var) from `noreply@sicusmedia.com`, with `reply_to` set to the submitter so the team can reply directly to the lead. Note: `api/subscribe.js` still sends from `hello@sicusmedia.com` intentionally — its email invites recipients to reply for a strategy call, so it must not use `noreply@`.
